...
export default function ChangeAddressPage() {
  ...
  const [, setConnectionStatus] = useState<"unknown" | "online" | "offline">(
    "unknown",
  );

  // Parsed address (live)
  const parsedAddr = useMemo(
    () => parseModbusAddress(startAddrRaw),
    [startAddrRaw],
  );
  ...
  useEffect(() => {
    if (regSplitMode === "split") {
      setRegArr(
        regHiArr.map((hi, i) => combineBytes(hi, regLoArr[i], endianness)),
      );
    }
  ...
  useEffect(() => {
    if (regSplitMode === "combined") {
      setRegHiArr(
        regArr.map((v) => (endianness === "big" ? (v >> 8) & 0xff : v & 0xff)),
      );
      setRegLoArr(
        regArr.map((v) => (endianness === "big" ? v & 0xff : (v >> 8) & 0xff)),
      );
    }
  ...
  const handleSend = useCallback(async () => {
    ...
    const reqBody = {
      ...
      slaveAddress: slave,
      functionCode:
        fc === "fc05" ? 5 : fc === "fc06" ? 6 : fc === "fc15" ? 15 : 16,
      address: addr,
      timeout: 1000,
    } as WriteConfig;
    ...
    try {
      ...
      if (data.success) {
        ...
        setResult({
          success: true,
          message: d.message || t("change_id_success"),
        });
      } else {
        setResult({
          success: false,
  ...
  const tabActive = (mode: FCMode) =>
    fc === mode
      ? "border-[2px] border-[#185FA5] dark:instrument-accent bg-[#E6F1FB] dark:instrument-accent/30"
      : "border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50";

  const tabTextPrimary = (mode: FCMode) =>
    fc === mode
      ? "text-[#0C447C] dark:instrument-accent"
      : "text-slate-900 dark:text-slate-100";
  const tabTextSecondary = (mode: FCMode) =>
    fc === mode
      ? "text-[#185FA5] dark:instrument-accent"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-app-muted">{t("change_id_subtitle")}</p>
        </div>
      </div>
      ...
      {scannedDevices.length > 0 && (
        <div className="instrument-panel p-4">
          <div className="flex items-center gap-2 mb-3">
            <CheckCircle2 className="w-4 h-4 instrument-accent dark:instrument-accent" />
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
              {t("change_id_scanned_title")}
            </span>
      ...
      {scannedDevices.length === 0 && (
        <div className="p-4 instrument-panel bg-instrument-accent/5 border-instrument-accent/20">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm text-app-text font-medium">
                {t("change_id_no_scanned")}
              </p>
              <p className="text-sm text-app-muted mt-1">
                {t("change_id_scan_hint")}{" "}
                <Link
                  href="/scan"
                  className="underline text-instrument-accent hover:opacity-80"
                >
                  {t("change_id_scan_link")}
                </Link>
              </p>
            </div>
          </div>
      ...
      <div className="flex gap-4">
        ...
        <div className="flex flex-col gap-2 w-44 flex-shrink-0">
          {[
            ...
            <button
              ...
              className={`cursor-pointer instrument-panel py-3 px-3 text-left transition-all ${tabActive(tab.mode)}`}
            >
              <div
                className={`text-base font-medium ${tabTextPrimary(tab.mode)}`}
              >
                {tab.code}
              </div>
              <div
                className={`text-[10px] mt-0.5 leading-tight ${tabTextSecondary(tab.mode)}`}
              >
                {tab.desc}
              </div>
            </button>
        ...
        {/* ─── Right: Main Panel ─── */}
        <div className="flex-1 instrument-panel p-5">
          ...
          <div className="flex items-center justify-end gap-2 mb-4">
            ...
            <button
              onClick={() =>
                setEndianness(endianness === "big" ? "little" : "big")
              }
              className="flex items-center gap-1.5 px-3 py-1.5 instrument-input border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all"
            >
          ...
          <div className="grid grid-cols-[80px_1fr_1fr] gap-3 mb-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSlaveId(
                    Math.max(1, Math.min(247, Number(e.target.value) || 1)),
                  )
                }
                className="bg-white dark:bg-slate-800 w-full px-3 py-2 instrument-input border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-[#185FA5]/40"
              />
            </div>
            <div>
              <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">
                {parsedAddr.notation} → frame address ={" "}
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 py-2.5 instrument-input text-sm font-medium transition-all ${
                    coilState === 1
                      ? "border-2 border-[#185FA5] dark:instrument-accent bg-[#E6F1FB] dark:instrument-accent/30 text-[#0C447C] dark:instrument-accent"
            ...
            <div className="mb-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`px-3 py-1 instrument-input text-xs font-medium transition-all ${
              ...
              {inputMode06 === "combined" ? (
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="px-3 py-2 instrument-input bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-mono text-sm">
                      0x{h(val06, 4)}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className={`bg-white dark:bg-slate-800 w-full px-3 py-2 instrument-input border text-slate-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-[#185FA5]/40 ${
                        hi06 > 255
              ...
              <div className="flex flex-wrap gap-2">
                {coilArr.map((v, i) => (
                  <button
                    ...
                    className={`px-3 py-1.5 instrument-input text-xs font-medium transition-all ${
                      v
                        ? "border-2 border-[#185FA5] dark:instrument-accent bg-[#E6F1FB] dark:instrument-accent/30 text-[#0C447C] dark:instrument-accent"
                        : "border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                    }`}
              ...
              <div className="flex flex-col gap-2">
                {regArr.map((v, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-mono min-w-[60px]">
                      Addr {parsedAddr.raw + i}
                    </span>
                    {regSplitMode === "combined" ? (
                      <>
                        <input
                          ...
                          className="bg-white dark:bg-slate-800 flex-1 px-3 py-2 instrument-input border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-[#185FA5]/40"
                        />
                        <span className="text-xs text-slate-400 dark:text-slate-500 font-mono w-16">
                          0x{h(v, 4)}
                        </span>
                      </>
                    ) : (
                      <>
                        <input
                          ...
                          value={regHiArr[i]}
                          onChange={(e) =>
                            updateRegHi(i, Number(e.target.value))
                          }
                          className={`bg-white dark:bg-slate-800 w-20 px-2 py-2 instrument-input border text-slate-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-[#185FA5]/40 text-xs ${
                            regHiArr[i] > 255
                              ? "border-red-500 dark:border-red-400 bg-red-50 dark:bg-red-900/20"
                              : "border-slate-300 dark:border-slate-600"
                          }`}
                        ...
                        <input
                          ...
                          value={regLoArr[i]}
                          onChange={(e) =>
                            updateRegLo(i, Number(e.target.value))
                          }
                          className={`bg-white dark:bg-slate-800 w-20 px-2 py-2 instrument-input border text-slate-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-[#185FA5]/40 text-xs ${
                            regLoArr[i] > 255
                              ? "border-red-500 dark:border-red-400 bg-red-50 dark:bg-red-900/20"
          ...
          {/* ─── Frame Monitor ─── */}
          <div className="instrument-panel bg-app-surface p-3 mb-4">
            <div className="flex items-center justify-between mb-2">
              <div className="text-[11px] text-app-muted uppercase tracking-wider">
                Raw Frame Monitor (hex)
              </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {frameSegments.map((seg, idx) => (
                <div key={idx} className="flex items-center">
                  <div
                    className={`inline-flex items-center gap-1 px-2 py-1 instrument-input border text-xs font-mono ${seg.color}`}
                  >
          ...
          <button
            ...
            className="w-full py-3 text-sm font-medium instrument-input border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 disabled:bg-slate-100 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
          >
            {changing ? (
              <>
          ...
          {result && (
            <div
              className={`mt-4 p-3 instrument-input border flex items-center gap-3 ${
                result.success
                  ? "bg-emerald-50 dark:instrument-accent/20 instrument-accent dark:instrument-accent"
                  : "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800"
              }`}
            >
              {result.success ? (
                <CheckCircle2 className="w-5 h-5 instrument-accent dark:instrument-accent flex-shrink-0" />
              ) : (
              ...
              <span
                className={
                  result.success
                    ? "instrument-accent dark:instrument-accent text-sm"
      ...
      {logs.length > 0 && (
        <div className="instrument-panel p-5">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">
            Log
          </div>
          <div className="flex flex-col gap-2">
            {logs.map((entry) => (
              <div
                key={entry.id}
                className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 instrument-input px-3 py-2 text-xs"
              >
